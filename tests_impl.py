...
def unconditional_relevance(
    ...
    W = float(T * (g2.T @ np.linalg.inv(Sigma_g2) @ g2))
...
def conditional_relevance(
    ...
    Omega11_inv = np.linalg.inv(Omega11 + ridge * anp.eye(m1))
    G2 = G[f2_indexes, :]
    G1 = G[f1_indexes, :]
    ...
    W = float(T) * gdelta_vec.T @ np.linalg.inv(Sigma_g_delta) @ gdelta_vec
...
def partial_unconditional_relevance(
    ...
    моментов f2 "не может сам по себе задавать направления" для локального минимума изменяя параметры из набора A, а значит 
    не несет информации для оценки параметров theta из набора A. Таким образом, моменты f2 "нерелевантны" для оценки параметров theta_a 
    сами по себе.
    ...
    W = float(T * (g2a.T @ np.linalg.inv(Sigma_g2a) @ g2a))
...
def partial_conditional_relevance(
    ...
    точность оценок в координатах параметров theta_a (subset A).
    ...
    # 3) Находим g_delta_a_vec
    vprint('[ESTIMATING g_delta: ]', end='')
    Omega21 = Omega[f2_indexes][:,f1_indexes]
    Omega11 = Omega[f1_indexes][:,f1_indexes]
    Omega11_inv = np.linalg.inv(Omega11 + ridge * np.eye(m1))
    G2 = G[f2_indexes, :]
    G1 = G[f1_indexes, :]
    Gdelta_T = G2 - Omega21 @ Omega11_inv @ G1
    R_A = np.eye(k)[:, a_indexes].T
    gdelta_vec = vec(Gdelta_T.T)
    gdelta_a_vec = np.kron(np.eye(m2), R_A) @ gdelta_vec
    vprint('[DONE]')
    ...
    # 7) Строим финальную статистику W
    W = float(T) * gdelta_a_vec.T @ np.linalg.inv(Sigma_g_delta_a) @ gdelta_a_vec
